...
for x in LR:
    if len(L_avl) == 0:
        L_avl[x[0]]=x[1]
        continue
    key = L_avl.upper_bound(x[0])
    if key == None:
        L_avl[x[0]] = x
        key = L_avl.lower_bound(x[1])    
            if key < x[0] and L_avl[key] < x[1]:
                L_avl[key] = x[1]
            elif L_avl[key] >= x[1]:
                pass
            else:
    ...
    if L_avl[key] >= x[0] and L_avl[key] < x[1]:
        L_avl[key] = x[1]
    elif L_avl[key] >= x[1]:
        pass
    else:
...
for  i in range(len(L_avl)):
    key = L_avl.find_kth_element(i)
    print(key,L_avl[key])